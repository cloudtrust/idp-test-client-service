...
RUN	mvn clean package; \
	...
	mv target/IdpTestClient.jar /opt/idp-test-client-run/; \
...
RUN install -v -o root -g root -m 644 -d /etc/systemd/system/idp-test-client.service.d; \
    ...
    install -v -o SOresource -g SOresource -m deploy/common/opt/runIdpTestClient.sh /opt/runIdpTestClient.sh
